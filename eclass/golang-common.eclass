			...
			for i in ${!GOLANG_PKG_DEPENDENCIES[@]} ; do
				...
				case ${DEPENDENCY[importpathalias]} in
					gopkg.in*)
						...
						mkdir -p "${GOPATH}/src/${DEPENDENCY[importpathalias]%/*}/${DEPENDENCY[project_name]}" || die
						#einfo "\n${GOPATH}/src/${DEPENDENCY[importpathalias]%/*}/${DEPENDENCY[project_name]}"
						;;
					*)
				...
				ebegin "${DEPENDENCY[project_name]}-${DEPENDENCY[revision]}*"
				eend
				...
				case ${DEPENDENCY[host]} in
					...
					rsc.io*)
						...
						;;
					gopkg.in*)
						einfo "${DEPENDENCY[importpathalias]}"
						ebegin "${message}"
							mv ${DEPENDENCY[project_name]%%.*}-${DEPENDENCY[revision]}*/* "${GOPATH}"/src/${destdir} || die
						eend
						;;
