	...
	for i in ${!GOLANG_PKG_DEPENDENCIES[@]} ; do
		...
		case ${DEPENDENCY[importpath]} in
			...
			code.google*)
				...
				;;
			golang.org*)
				SRC_URI+=" https://github.com/golang/${DEPENDENCY[project_name]}/archive/${DEPENDENCY[revision]}${GOLANG_PKG_ARCHIVESUFFIX} -> ${DEPENDENCY[importpath]//\//-}-${DEPENDENCY[revision]}${GOLANG_PKG_ARCHIVESUFFIX}"
				;;
			gopkg.in*)
				SRC_URI+=" https://github.com/${DEPENDENCY[author_name]}/${DEPENDENCY[project_name]%%.*}/archive/${DEPENDENCY[revision]}${GOLANG_PKG_ARCHIVESUFFIX} -> ${DEPENDENCY[importpath]//\//-}-${DEPENDENCY[revision]}${GOLANG_PKG_ARCHIVESUFFIX}"
				;;
			rsc.io*)
				SRC_URI+=" https://github.com/rsc/${DEPENDENCY[project_name]}/archive/${DEPENDENCY[revision]}${GOLAND_PKG_ARCHIVESUFFIX}.tar.gz -> ${DEPENDENCY[importpath]//\//-}-${DEPENDENCY[revision]}${GOLANG_PKG_ARCHIVESUFFIX}"
				;;
